...
class _Peripheral {
        ...
        let observable = delegateWrapper
            ...
            .map { [weak self] (cbCharacteristic, error) -> _Characteristic in
                guard let strongSelf = self,
                      let characteristic = characteristic ?? _Characteristic(characteristic: cbCharacteristic, peripheral: strongSelf) else {
    ...
    func observeNotifyValue(for characteristic: _Characteristic) -> Observable<_Characteristic> {
        return delegateWrapper.peripheralDidUpdateNotificationStateForCharacteristic
            ...
            .map { [weak self] (cbCharacteristic, error) -> _Characteristic in
                guard let strongSelf = self,
                      let characteristic = _Characteristic(characteristic: cbCharacteristic, peripheral: strongSelf) else {
                    throw _BluetoothError.destroyed
                }
                if let error = error {
    ...
    func observeWrite(for descriptor: _Descriptor? = nil) -> Observable<_Descriptor> {
        let observable = delegateWrapper
            ...
            .map { [weak self] (cbDescriptor, error) -> _Descriptor in
                guard let strongSelf = self,
                      let descriptor = descriptor ?? _Descriptor(descriptor: cbDescriptor, peripheral: strongSelf) else {
                          throw _BluetoothError.destroyed
                      }
                if let error = error {
